...
func businessLogic(srv *atopraft.BaseServer, args atopraft.SrvArgs, reply *atopraft.SrvReply, logHeader string) {
	...
	case OP_GET:
		...
		reply.Value = &v2
	case OP_PUT:
		store2[payload.Key] = payload.Value
		v2 := KVReplyValue("")
		reply.Value = &v2
	case OP_APPEND:
		v, ok := store2[payload.Key]
		...
		store2[payload.Key] = v + payload.Value
		v2 := KVReplyValue("")
		reply.Value = &v2
	}
}
...
func (kv *KVServer) ServerStatus(args *atopraft.ServerStatusArgs, reply *atopraft.ServerStatusReply) {
