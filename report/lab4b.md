...
- Reconfigurations are preformed one by one instead of going to the latest one directly, which is a lot easier.
- By comparing the current configuration and the new configuration, we can know what inShard and outShard are. The server can get inShard data from servers in *inGroup*, and it needs to wait for servers in *outGroup* to fetch its outShard data. Those two groups can also be easily computed.

2️⃣ T1 starts a `START` log entry and waits for it to be consumed.

T2 consumes the `START` entry: sets the reconfiguration tuple to be `(nextNum, START)`, and rejects any client operation whose key is in outShard.

3️ T1 fetches all inShard data, for every shard in inShard:

- T1 starts a new goroutine T3 to fetch the shard data from the corresponding group, which is one of the group in inGroup.
- A server is only allowed to return data if it is at least at `(nextNum, START)`.
- If T3 gets the data successfully, it starts a `PREPARE_SINGLE` log entry containing the inShard data and waits for it to be consumed.
- T2 consumes the `PREPARE_SINGLE` entry: adds the inShard data to the KV Store, and allows client operations whose keys are in this shard.

T1 waits for all the goroutines to complete successfully.

4️⃣ T1 starts a `PREPARE` log entry and waits for it to be consumed.

T2 consumes the `PREPARE` entry: sets the reconfiguration tuple to be `(nextNum, PREPARE)`.

5️⃣ T1 waits for all groups in outGroup to be at least `(NextNum, PREPARE)`, which means all these groups got their inShard data.

6️⃣ T1 starts a `COMMIT` log entry and waits for it to be consumed.

T2 consumes the `COMMIT` entry: deletes the oldShard data from the KV Store, sets `Config` to the new configuration, and sets the reconfiguration tuple to be `(nextNum, COMMIT)`.
...
- The client thinks its initial request failed, thus resending the request to the new group. The new group consumes it again, causing wrong results.
...
However, when we want the server to return state data directly instead of going through Raft, inconsistency may happen. Let's consider the reconfiguration protocol. In step 3️: "A server is only allowed to return data if it is at least at `(nextNum, START)`." If no such condition, the returned shard data may change in two consecutive fetches because the server hasn't entered `START` phase (the log entry is yet to be applied), which means it currently does not reject client operations whose keys are in its outShard.
...
- The three challenge tests are not optional. They are important guides to follow if you want to pass all non-challenge tests 100% correctly.
- I have refactored a lot during debugging this lab.
  - After discussing it a lot in previous lab reports, I finally implemented a feature flag to enable fine-grained control about what components are allowed to log. For example, we can turn off the shard controller clerk's log while turning on the sharded K/V clerk's log. The code is tedious: add a boolean variable to the config, and check it before every logging statement.
  - The previous config only consists constant variables, which is impossible to apply different configs to multiple components. Now I define a struct containing all these variables, and give instances of the config struct to each component, so that they can use different config at their will.
- I gave up adapting the code to GitHub Actions. The race detector imposes a great cost on both CPU and memory, but the machines of GitHub Actions are simply not as performant as my computer. So, I remove the `-race` flag when running tests of this lab on GitHub Actions. Note that I always enable the flag when testing on my computer, and have confirmed that there are no data race happened.
